...
(function ($) {
    ...
    $.fn.idleTimeout = function (userRuntimeConfig) {
        ...
        var defaultConfig = {
            ...
            debounceMillis: 20000,  //throttle for user action in milliseconds. 20000 = 20 seconds.

            onBeforeShowWarningDialog: function () { }, // Callback right before the warning dialog is shown so that other dialogs
                                                        // may be hidden temporarily

            onAfterHideWarningDialog: function () { } // Callback right after the warning dialog is hidden so that other dialogs that
                                                      // were temporarily hidden may be shown again
            },
        ...
        activityDetector = function () {
            $("body").on(currentConfig.activityEvents, JD.debounce(function () {
                if (!currentConfig.enableDialog || (currentConfig.enableDialog && isDialogOpen() !== true)) {
                    startIdleTimer();
                }
                $.get(currentConfig.sessionKeepAliveUrl);
            }, currentConfig.debounceMillis, true));
        };
        ...
        openWarningDialog = function () {
            ...
            currentConfig.onBeforeShowWarningDialog.call();

            var dialogContent = "<div id='idletimer_warning_dialog'><p>" + currentConfig.dialogText + "</p><p class='idle-timeout-inline'>" + currentConfig.dialogTimeRemaining + ": <div class='idle-timeout-inline' id='countdownDisplay'></div></p></div>";
        ...
        destroyWarningDialog = function () {
            ...
            currentConfig.onAfterHideWarningDialog.call();
        ...
        countdownDisplay = function () {
            ...
            remainingTimer = setInterval(function () {
                // This will display the remaining time more consistently
                // and without encountering an issue with NaN
                var remainingTime = new Date();
                remainingTime.setHours(0, 0, dialogDisplaySeconds);
                $("#countdownDisplay").html(remainingTime.toTimeString().substr(3, 5));
